	...
	tables.forEach(table => {
		if (!table.isValid) {
			validationErrors.push(
				`Inconsistência na tabela ${table.name}: backup tem ${table.recordCount} registros`
			)
		}
	})
