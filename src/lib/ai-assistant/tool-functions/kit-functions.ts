...
export async function listProductKits(args: { includeInactive?: boolean }) {
  try {
    ...
    if (!kits || kits.length === 0) {
      return {
        success: true,
        message: "Nenhum kit cadastrado ainda. Você pode criar um kit para combos promocionais que os mercados oferecem!",
        kits: [],
      };
    }
 ...
 */
export async function calculateKitSavings(args: {
  kitName: string;
  paidPrice?: number;
  marketName?: string;
}) {
  try {
    ...
    message += `💰 **Preço sugerido (produtos separados):** R$ ${priceInfo.totalPrice.toFixed(
      2
    )}\n`;
    ...
    if (paidPrice) {
      ...
      if (savings > 0) {
        ...
        message += `🎉 **Economia:** R$ ${savings.toFixed(2)} (${savingsPercent.toFixed(
          1
        )}%)\n\n`;
        message += `Parabéns! Você economizou comprando o combo!\n`;
      } else if (savings < 0) {
        message += `⚠️ **Você pagou:** R$ ${paidPrice.toFixed(2)}\n`;
        message += `❌ **Prejuízo:** R$ ${Math.abs(savings).toFixed(
          2
 ...
 */
export async function findSimilarKits(args: {
  searchTerm: string;
  context?: any;
}) {
  try {
  ...
  try {
    ...
    if (!kits || kits.length === 0) {
      return {
        success: true,
        message:
          "Você ainda não tem kits cadastrados. Crie kits para combos promocionais que os mercados oferecem!",
      };
    }
    ...
    const kitAvailability = await Promise.all(
      kits.map(async (kit) => {
        const stock = await productKitService.checkKitStockAvailability(
          kit.kitProductId
        );
        return {
          kit,
    ...
    const unavailableKits = kitAvailability.filter(
      (k) => k.stock.availableQuantity === 0
    );
    ...
    if (availableKits.length > 0) {
      ...
      availableKits.forEach((k, i) => {
        message += `${i + 1}. **${k.kit.kitProduct.name}** - ${k.stock.availableQuantity
          } kit(s) disponível(is)\n`;
        message += `   Produtos: ${k.kit.items
...
export async function compareKitPrices(args: { kitName: string }) {
  try {
    ...
    message += `\n🏆 **Melhor opção:** ${bestMarket.market} por R$ ${bestMarket.lastPrice.toFixed(
      2
    )}`;
