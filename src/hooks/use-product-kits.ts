...
export function useKitNutrition(kitProductId: string | null) {
  const [nutrition, setNutrition] = useState<AggregatedNutritionalInfo | null>(
    null
  );
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const loadNutrition = async () => {
    ...
    try {
      const response = await fetch(
        `/api/product-kits/${kitProductId}/nutrition`
      );
      const data = await response.json();
...
export function useKitStock(kitProductId: string | null) {
  ...
  const consumeFromStock = async (quantity: number, reason?: string) => {
      ...
      try {
        const response = await fetch(
          `/api/product-kits/${kitProductId}/stock/consume`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
