...
export function NutritionalInfoDialog({ productId, onSuccess, onCancel, onShowScanner }: NutritionalInfoDialogProps) {
	...
	const _handleScanResult = async (imageData: string) => {
		try {
			setIsAnalyzing(true)
	...
	return (
		<>
			...
			<div className="flex flex-col sm:flex-row gap-3 mb-6">
				<Button variant="outline" onClick={onShowScanner} disabled={isSubmitting || isAnalyzing} className="flex-1">
					<QrCode className="size-4 mr-2" />
					Scanner de Rótulo
				</Button>
				<Button variant="outline" disabled={true} className="flex-1 opacity-50" title="Em breve: Análise com IA">
			...
			<div className="space-y-6 mb-6">
				<NutritionalInfoForm initialData={formData} onDataChange={setFormData} />
			</div>

			{/* Botões de ação na base - fora do scroll */}
			<div className="flex justify-end gap-3 pt-6 border-t bg-background">
				<Button variant="outline" onClick={onCancel} disabled={isSubmitting || isAnalyzing}>
					Cancelar
				</Button>
