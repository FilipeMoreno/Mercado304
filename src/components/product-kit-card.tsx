...
import type { KitStockInfo, ProductKitWithItems } from "@/types/product-kit"
...
export function ProductKitCard({
	...
	const loadStockInfo = async () => {
		...
		try {
			const response = await fetch(`/api/product-kits/${kit.kitProductId}/stock`)
	...
	return (
		<Card className="w-full hover:shadow-md transition-shadow-sm">
			<CardHeader>
				<div className="flex items-start justify-between">
					<div className="flex items-center gap-2">
						...
						<div>
							<CardTitle className="text-lg">{kit.kitProduct.name}</CardTitle>
							{kit.description && <CardDescription className="mt-1">{kit.description}</CardDescription>}
						</div>
					</div>
			...
			<CardContent>
				...
				<div className="space-y-2">
					...
					<div className="space-y-1">
						{kit.items.map((item) => (
							<div key={item.id} className="flex items-center justify-between text-sm p-2 rounded-md bg-secondary/30">
								<div className="flex items-center gap-2">
				...
				{showStock && (
					<div className="mt-4 pt-4 border-t">
						{!stockInfo ? (
							<Button variant="outline" size="sm" onClick={loadStockInfo} disabled={isLoadingStock} className="w-full">
							...
							<div className="space-y-2">
								<div className="flex items-center justify-between">
									...
									{stockStatus && (
										<Badge
											variant={stockStatus.color as "default" | "secondary" | "destructive"}
								...
								{stockInfo.limitingProduct && (
									<p className="text-xs text-muted-foreground">Limitado por: {stockInfo.limitingProduct.name}</p>
								)}
							</div>
						)}
					</div>
			...
			<CardFooter className="flex gap-2">
				{onViewDetails && (
					<Button variant="outline" size="sm" onClick={() => onViewDetails(kit)} className="flex-1">
						Ver Detalhes
					</Button>
				)}
				{onEdit && (
					<Button variant="default" size="sm" onClick={() => onEdit(kit)} className="flex-1">
						Editar
					</Button>
				)}
			</CardFooter>
