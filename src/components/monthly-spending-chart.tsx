...
export function MonthlySpendingChart({ data, loading }: MonthlySpendingChartProps) {
	if (loading) {
		return (
			<Card>
				<CardHeader>
			...
			<CardContent>
				<ResponsiveContainer width="100%" height={300}>
					<LineChart data={data}>
						...
						<XAxis dataKey="month" tickFormatter={(value) => format(new Date(value), "MMM/yy", { locale: ptBR })} />
						...
						/>
						<Line type="monotone" dataKey="totalSpent" stroke="#8884d8" strokeWidth={2} dot={{ fill: "#8884d8" }} />
					</LineChart>
				</ResponsiveContainer>
			</CardContent>
		</Card>
