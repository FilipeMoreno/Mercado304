...
import { AlertTriangle, Calendar, Clock, Package, Trash2, CheckCircle } from "lucide-react"
...
export function ExpirationAlerts({ data, loading, onRefresh }: ExpirationAlertsProps) {
  ...
  if (!data || (!data.alerts.expired.length && !data.alerts.expiringToday.length && 
                 !data.alerts.expiringSoon.length && !data.alerts.lowStock.length)) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
  ...
  const allAlerts = [
    ...data.alerts.expired.map((item: any) => ({ ...item, alertType: 'expired', priority: 3 })),
    ...data.alerts.expiringToday.map((item: any) => ({ ...item, alertType: 'today', priority: 2 })),
    ...data.alerts.expiringSoon.map((item: any) => ({ ...item, alertType: 'soon', priority: 1 })),
    ...data.alerts.lowStock.map((item: any) => ({ ...item, alertType: 'lowStock', priority: 0 }))
  ].sort((a, b) => b.priority - a.priority)

  const getAlertColor = (alertType: string) => {
    switch (alertType) {
      case 'expired': return 'border-l-red-500 bg-red-50'
      case 'today': return 'border-l-orange-500 bg-orange-50'
      case 'soon': return 'border-l-yellow-500 bg-yellow-50'
      case 'lowStock': return 'border-l-blue-500 bg-blue-50'
  ...
  const getAlertIcon = (alertType: string) => {
    switch (alertType) {
      ...
      case 'today': return <Calendar className="h-4 w-4 text-orange-600" />
      case 'soon': return <Clock className="h-4 w-4 text-yellow-600" />
      case 'lowStock': return <Package className="h-4 w-4 text-blue-600" />
  ...
  const getAlertMessage = (item: any, alertType: string) => {
    switch (alertType) {
      ...
      case 'today':
        return 'Vence hoje!'
      case 'soon':
        const daysLeft = Math.ceil((new Date(item.expirationDate).getTime() - new Date().getTime()) / (24 * 60 * 60 * 1000))
        ...
        <CardDescription>
          {data.stats.potentialWasteValue > 0 && (
            <span className="text-red-600">
              Risco de desperdício: R$ {data.stats.potentialWasteValue.toFixed(2)}
            </span>
          )}
        </CardDescription>
      </CardHeader>
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-start gap-3 flex-1">
                ...
                <div className="flex-1">
                  ...
                  {alert.product.brand && (
                    <p className="text-sm text-gray-500">{alert.product.brand.name}</p>
                  )}
