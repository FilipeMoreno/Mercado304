...
export function ShoppingSummary({ items, totalItems, completedItems }: ShoppingSummaryProps) {
	const totalEstimated = items.reduce((sum, item) => sum + item.quantity * (item.estimatedPrice || 0), 0)
		...
		<Card>
			...
			<CardContent>
				<div className="grid grid-cols-2 md:grid-cols-5 gap-4">
					...
					<div>
						<div className="text-2xl font-bold text-purple-600">R$ {totalEstimated.toFixed(2)}</div>
						<div className="text-sm text-gray-600">Total Estimado</div>
					</div>
					<div>
						<div className="text-2xl font-bold text-green-600">R$ {completedTotal.toFixed(2)}</div>
