...
export function KitCardEnhanced({ kit, onEdit, onDelete }: KitCardEnhancedProps) {
	...
	const { data: stockInfo, isLoading: isLoadingStock } = useProductKitStockQuery(loadStock ? kit.kitProductId : "")
	...
	return (
		<Card className="w-full hover:shadow-lg transition-all duration-200 border-2 hover:border-primary/50">
				...
				<div className="flex items-start justify-between">
					<div className="flex items-center gap-2 flex-1">
						...
						<div className="flex-1 min-w-0">
							<CardTitle className="text-lg line-clamp-1">{kit.kitProduct.name}</CardTitle>
							{kit.description && <p className="text-sm text-muted-foreground mt-1 line-clamp-2">{kit.description}</p>}
						</div>
					</div>
				...
				<div>
					...
					<div className="space-y-1">
						...
						{kit.items.length > 2 && !expanded && (
							<Button variant="ghost" size="sm" onClick={handleExpand} className="w-full text-xs">
			...
			<CardFooter className="flex gap-2 pt-3">
				<Button variant="default" size="lg" onClick={handleViewDetails} className="flex-1">
				...
				<DropdownMenu>
					...
					<DropdownMenuContent align="end">
						...
						<DropdownMenuSeparator />
						<DropdownMenuItem onClick={() => onDelete?.(kit)} className="text-destructive focus:text-destructive">
