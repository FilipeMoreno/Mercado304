...
const allergenIconMap: { [key: string]: { icon: React.ComponentType<any>; color: string; bgColor: string } } = {
	...
	peixe: {
		...
		bgColor: "bg-cyan-50 border-cyan-200 dark:bg-cyan-950 dark:border-cyan-800",
	},
	crustáceos: {
		icon: Shell,
...
const AllergenIcon = ({
	...
	const iconClass = isContains ? allergenData.color : "text-yellow-700 dark:text-yellow-300"
		...
		<TooltipProvider>
			<Tooltip>
				...
				<TooltipContent side="top" className="font-medium">
					<p>{allergen}</p>
					<p className="text-xs text-muted-foreground">{isContains ? "Contém" : "Pode conter"}</p>
				</TooltipContent>
			</Tooltip>
		</TooltipProvider>
	)
			...
			{allergensContains.length > 0 && (
				<div className="space-y-3">
					<div className="flex items-center gap-2">
						...
						<h4 className="text-sm font-semibold text-red-600 dark:text-red-400 uppercase tracking-wide">Contém</h4>
					</div>
