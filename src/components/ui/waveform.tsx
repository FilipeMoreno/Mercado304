	...
	useEffect(() => {
		...
		const animate = (currentTime: number) => {
			...
			while (
				...
				(barsRef.current[barsRef.current.length - 1] && barsRef.current[barsRef.current.length - 1]!.x < rect.width)
			) {
...
export const AudioScrubber = ({
	...
	const waveformData = data.length > 0 ? data : Array.from({ length: 100 }, () => 0.2 + Math.random() * 0.6)
...
export const LiveMicrophoneWaveform = ({
			...
			if (dataToRender.length > 0) {
				...
				for (let i = 0; i < barCount; i++) {
					...
					if (active) {
						...
						dataIndex = Math.max(
							0,
							Math.min(dataToRender.length - 1, dataToRender.length - 1 - i - Math.floor(offsetInBars)),
						)
					}
	...
	return (
		<div
			...
			aria-label={!active && historyRef.current.length > 0 ? "Drag to scrub through recording" : undefined}
