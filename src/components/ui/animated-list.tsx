...
export function AnimatedList({
  ...
  const handleItemSelect = useCallback((id: string | number, data?: any) => {
    if (onItemSelect) {
  ...
  const getSwipeActions = useCallback((item: ListItem) => {
    const leftActions = []
    const rightActions = []
          ...
          return (
            <div key={item.id} className={cn("relative", itemClassName)}>
              {enableSwipeActions ? (
                ...
                >
                  <div onClick={() => handleItemSelect(item.id, item.data)}>
                    {item.content}
                  </div>
                </SwipeableCard>
              ) : (
