...
import { Camera, RotateCcw, Upload, X, Zap, ZapOff } from "lucide-react"
...
export function ProductPhotoCapture({ onPhotoCapture, onClose, isProcessing = false }: ProductPhotoCaptureProps) {
		...
		<Card className="fixed inset-0 z-50 bg-black/95 flex flex-col">
			...
			<div className="flex items-center justify-between p-4 bg-black/50">
				...
				<div className="flex items-center gap-2">
					...
					<Button variant="ghost" size="sm" onClick={handleClose} className="text-white" disabled={isProcessing}>
						<X className="size-5" />
					</Button>
				</div>
			</div>
					...
					<>
						...
						<div className="p-4 bg-black/50">
							<div className="flex items-center justify-between max-w-md mx-auto">
								{/* Flash */}
								<Button variant="ghost" size="lg" onClick={toggleFlash} className="text-white" disabled={isProcessing}>
									{flashEnabled ? <Zap className="size-6" /> : <ZapOff className="size-6" />}
								</Button>
