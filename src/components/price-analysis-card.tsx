    ...
    <Card className={className}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="text-center">
            <p className="text-2xl font-bold">R$ {analysis.trends.avgPrice?.toFixed(2)}</p>
            <p className="text-xs text-muted-foreground">Preço médio</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold">R$ {analysis.trends.priceRange?.toFixed(2)}</p>
            <p className="text-xs text-muted-foreground">Variação</p>
          </div>
          <div className="text-center flex items-center justify-center">
            {getTrendIcon(analysis.trends.recentTrend)}
            <span className={`ml-1 text-sm font-medium ${getTrendColor(analysis.trends.recentTrend)}`}>
              {analysis.trends.recentTrend === 'up' ? 'Subindo' : 
               analysis.trends.recentTrend === 'down' ? 'Descendo' : 'Estável'}
            </span>
          </div>
        </div>
