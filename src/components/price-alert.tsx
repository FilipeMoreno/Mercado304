		...
		>
						...
						<div className="space-y-2">
							<div className="flex items-center gap-2 text-sm">
								...
								<span className="font-medium">
									{alertData.details.suggestedMarket?.name}
								</span>
							...
							<div className="grid grid-cols-2 gap-4 text-xs text-gray-600">
								<div>
									...
									<div className="text-green-600 font-bold">
										R$ {alertData.details.savings?.toFixed(2)} (
										{alertData.details.savingsPercent?.toFixed(1)}%)
									</div>
								</div>
								<div>
									<span className="font-medium">Baseado em:</span>
									<div>
										{alertData.details.historicalPurchases} compras anteriores
									</div>
								</div>
							</div>
						</div>
						...
						<div className="space-y-2">
							<div className="grid grid-cols-2 gap-4 text-xs text-gray-600">
								<div>
									...
									<div className="text-red-600 font-bold">
										R$ {alertData.details.currentPrice.toFixed(2)}
									</div>
								</div>
								<div>
									<span className="font-medium">Média histórica:</span>
									<div className="text-gray-700 font-bold">
										R$ {alertData.details.averagePrice?.toFixed(2)}
									</div>
								</div>
							</div>
							...
							<div className="text-xs text-gray-600">
								...
								<span className="text-red-600 font-bold ml-1">
									+{alertData.details.percentDifference?.toFixed(1)}% acima da
									média
								</span>
							</div>
						</div>
			...
			{onClose && (
				<Button
					variant="ghost"
					size="icon"
					className="absolute top-2 right-2 h-auto w-auto"
					onClick={onClose}
				>
					<X className="h-4 w-4" />
				</Button>
