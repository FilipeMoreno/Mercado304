...
export default function NovaListaPage() {
	...
	const [relatedProductsVisibility, setRelatedProductsVisibility] = useState<
		boolean[]
	...
	const removeItem = (index: number) => {
		if (items.length > 1) {
			...
			setRelatedProductsVisibility(
				relatedProductsVisibility.filter((_, i) => i !== index),
			);
			setPriceAlertVisibility(
				priceAlertVisibility.filter((_, i) => i !== index),
			);
		}
	};
