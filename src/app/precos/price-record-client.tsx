...
export function PriceRecordClient({ initialProducts, initialMarkets }: PriceRecordClientProps) {
  ...
  const filteredRecords = priceRecords.filter(record => {
    ...
    const matchesMarket = !selectedMarket || selectedMarket === "all" || record.market.toLowerCase().includes(selectedMarket.toLowerCase());
    const matchesProduct = !selectedProduct || selectedProduct === "all" || record.product.toLowerCase().includes(selectedProduct.toLowerCase());
  ...
  const stats = {
    ...
    avgPrice: priceRecords.length > 0 
      ? priceRecords.reduce((sum, r) => sum + r.price, 0) / priceRecords.length 
      : 0
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Registre preços que você viu mas não comprou para futuras comparações
          </p>
        </div>
      </div>
                ...
                <div>
                  ...
                  <Select value={selectedMarket} onValueChange={setSelectedMarket}>
                    ...
                    <SelectContent>
                      ...
                      {Array.from(new Set(priceRecords?.map(r => r.market) || [])).filter(Boolean).map(market => (
                ...
                <div>
                  ...
                  <Select value={selectedProduct} onValueChange={setSelectedProduct}>
                    ...
                    <SelectContent>
                      ...
                      {Array.from(new Set(priceRecords?.map(r => r.product) || [])).filter(Boolean).map(product => (
