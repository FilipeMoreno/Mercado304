			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
				{isLoading ? (
					...
					<>
						<Card>
							<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
								<CardTitle className="text-sm font-medium">Total Desperdiçado</CardTitle>
								<TrendingUp className="h-4 w-4 text-red-600" />
							</CardHeader>
							<CardContent>
								<div className="text-2xl font-bold text-red-600">R$ {stats?.totalValue.toFixed(2) || "0.00"}</div>
								<p className="text-xs text-muted-foreground">Em {stats?.totalCount || 0} registros</p>
							</CardContent>
						</Card>
						<Card>
							<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
								<CardTitle className="text-sm font-medium">Itens Desperdiçados</CardTitle>
						...
						<Card>
							...
							<CardContent>
								<div className="text-2xl font-bold">{stats?.totalQuantity.toFixed(1) || "0"}</div>
								<p className="text-xs text-muted-foreground">kg/litros desperdiçados</p>
							</CardContent>
						</Card>
					</>
				)}
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
						...
						<CardContent>
							<div className="space-y-3">
								{stats.reasonStats.slice(0, 5).map((stat, index) => {
									...
									return (
										<div key={stat.wasteReason} className="flex items-center justify-between">
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>Impacto financeiro do desperdício</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="space-y-4">
								<div className="flex justify-between items-center p-3 bg-red-50 rounded-lg">
									<span className="text-sm font-medium text-red-800">Valor Total</span>
									<span className="text-lg font-bold text-red-600">R$ {stats.totalValue.toFixed(2)}</span>
								</div>
								<div className="flex justify-between items-center p-3 bg-orange-50 rounded-lg">
									<span className="text-sm font-medium text-orange-800">Valor Médio por Item</span>
									<span className="text-lg font-bold text-orange-600">
										R$ {stats.totalCount > 0 ? (stats.totalValue / stats.totalCount).toFixed(2) : "0.00"}
									</span>
								</div>
								<div className="flex justify-between items-center p-3 bg-yellow-50 rounded-lg">
									<span className="text-sm font-medium text-yellow-800">Economia Potencial</span>
									<span className="text-lg font-bold text-yellow-600">-R$ {stats.totalValue.toFixed(2)}</span>
								</div>
								{stats.totalCount > 0 && (
									<div className="mt-4 p-4 bg-gray-50 rounded-lg">
										<h4 className="text-sm font-medium text-gray-700 mb-2">Dicas para Reduzir</h4>
										<ul className="text-xs text-gray-600 space-y-1">
											<li>• Monitore datas de validade regularmente</li>
											<li>• Use o sistema PEPS (Primeiro que Entra, Primeiro que Sai)</li>
							...
							<CardContent className="p-6">
								<div className="flex items-start justify-between">
									<div className="flex-1">
										<div className="flex items-start gap-4">
											<div className="flex-1">
												<h3 className="font-semibold text-lg text-gray-900">{record.productName}</h3>
												...
												<div className="flex items-center gap-2 mt-2">
													<Badge
														variant={wasteReasonColors[record.wasteReason as keyof typeof wasteReasonColors] as any}
													>
													...
													{record.brand && <Badge variant="outline">{record.brand}</Badge>}
												</div>
											</div>
										</div>
									</div>
...
function WasteForm({ initialData, onSubmit, onCancel }: WasteFormProps) {
	...
	return (
		<form onSubmit={handleSubmit} className="space-y-4">
			...
			<div className="grid grid-cols-2 gap-4">
				<div>
					<label className="block text-sm font-medium mb-2">Data do Desperdício *</label>
					<Input
						type="date"
	...
	return (
		<div className="space-y-6">
			<div className="grid grid-cols-2 gap-6">
				...
				<div>
					<h4 className="font-medium text-gray-900 mb-2">Informações do Desperdício</h4>
					<div className="space-y-2 text-sm">
						<div>
							<span className="text-gray-600">Motivo: </span>
							<Badge variant={wasteReasonColors[record.wasteReason as keyof typeof wasteReasonColors] as any}>
								{wasteReasonLabels[record.wasteReason as keyof typeof wasteReasonLabels]}
							</Badge>
						</div>
						<div>
