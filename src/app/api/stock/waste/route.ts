...
export async function GET(request: Request) {
    ...
    `;
		...
		const periodLength =
			(dateFilter.date?.lte || new Date()).getTime() -
			(dateFilter.date?.gte || new Date()).getTime();
		...
		return NextResponse.json(
			{ error: "Erro ao buscar estatísticas de desperdício" },
			{ status: 500 },
		);
	}
}
...
export async function POST(request: Request) {
	try {
		...
		if (!stockItemId || !quantity || quantity <= 0) {
			return NextResponse.json(
				{ error: "stockItemId e quantity (> 0) são obrigatórios" },
				{ status: 400 },
			);
		}
		...
		if (!wasteReason) {
			return NextResponse.json(
				{ error: "wasteReason é obrigatório para desperdício" },
				{ status: 400 },
			);
		}
		...
		if (quantity > stockItem.quantity) {
			return NextResponse.json(
				{ error: `Quantidade insuficiente. Disponível: ${stockItem.quantity}` },
				{ status: 400 },
			);
		}
		...
		const response = await fetch(
			`${process.env.NEXTAUTH_URL || "http://localhost:3000"}/api/stock/history`,
			{
				method: "POST",
		...
		return NextResponse.json(
			{ error: "Erro ao registrar desperdício" },
			{ status: 500 },
		);
	}
}
