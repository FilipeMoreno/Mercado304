...
export async function GET(request: NextRequest) {
  try {
    ...
    const categoryWaste = currentMonthWaste.reduce((acc, waste) => {
    ...
    const wasteByReason = currentMonthWaste.reduce((acc, waste) => {
    ...
    const wasteByCategoryStats = Object.entries(categoryWaste).map(([category, count]) => ({
      category,
      count,
    ...
    const response = {
      ...
      wasteTrend,
      recentWaste,
      wasteByReason: Object.entries(wasteByReason).map(([reason, count]) => ({
        reason,
        count,
        percentage: ((count / totalWasteItems) * 100).toFixed(1)
      })).sort((a, b) => b.count - a.count),
      ...
      comparison: {
        ...
        percentageChange: previousMonthItems > 0 
