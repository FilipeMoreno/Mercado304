...
export async function GET(request: Request) {
	try {
		...
		switch (filter) {
			case "expired":
				filteredItems = itemsWithAlerts.filter(
					(item) => item.expirationStatus === "expired",
				);
				break;
			case "expiring":
				filteredItems = itemsWithAlerts.filter(
					(item) => item.expirationStatus === "expiring_soon",
				);
				break;
			case "low_stock":
				filteredItems = itemsWithAlerts.filter(
		...
		const stats = {
			...
			totalValue: stockItems.reduce(
				(sum, item) =>
					sum + (item.unitCost ? item.quantity * item.unitCost : 0),
				0,
			),
			expiredItems: itemsWithAlerts.filter(
				(item) => item.expirationStatus === "expired",
			).length,
			expiringSoon: itemsWithAlerts.filter(
				(item) => item.expirationStatus === "expiring_soon",
			).length,
			lowStockItems: itemsWithAlerts.filter(
				(item) => item.stockStatus === "low",
			).length,
			locations: Array.from(
				new Set(stockItems.map((item) => item.location).filter(Boolean)),
			),
		};
