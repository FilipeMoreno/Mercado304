...
export async function GET() {
  try {
    ...
    const expiringToday = stockItems.filter(item => 
      item.expirationDate && 
    ...
    const wasteValue = [...expiredItems, ...expiringToday, ...expiringSoon]
      .reduce((sum, item) => sum + (item.unitCost ? item.quantity * item.unitCost : 0), 0)
              ...
              if (item) {
                ...
                await prisma.stockItem.update({
                  ...
                  data: {
                    ...
                    isLowStock: item.product.hasStock && item.product.minStock ? 
                      newQuantity <= item.product.minStock : false
                  }
                })
