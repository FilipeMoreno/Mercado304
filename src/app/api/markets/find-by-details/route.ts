...
export async function POST(request: Request) {
  try {
    ...
    if (potentialMarkets.length > 1 && address) {
      console.log(`[MARKET_FIND_V2] Múltiplos mercados encontrados (${potentialMarkets.length}). Usando endereço para desambiguar: "${address}"`)
    ...
    console.log(`[MARKET_FIND_V2] Desambiguação pelo endereço falhou. Retornando o primeiro mercado encontrado como fallback: ${fallbackMarket.name}`)
