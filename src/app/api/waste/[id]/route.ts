    ...
    try {
		...
		const updatedRecord = await prisma.wasteRecord.update({
			...
			data: {
				...
				expirationDate: expirationDate ? new Date(expirationDate) : existingRecord.expirationDate,
				location,
				unitCost: unitCost ? parseFloat(unitCost) : existingRecord.unitCost,
				totalValue: totalValue ? parseFloat(totalValue) : existingRecord.totalValue,
				notes,
				category,
				brand,
				batchNumber,
