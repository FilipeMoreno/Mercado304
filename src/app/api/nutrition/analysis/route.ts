...
export async function GET(request: Request) {
	try {
		...
		nutritionalData.forEach((item) => {
			...
			if (nutrition.totalSugars && nutrition.totalSugars > 15)
				highSugarProducts++;
			...
			if (item.product?.category) {
				...
				categoryAnalysis[categoryKey].totalCalories +=
		...
		const healthiestProducts = productsWithScores
			.sort((a, b) => b.healthScore - a.healthScore)
			.slice(0, 5);

		const leastHealthyProducts = productsWithScores
		...
		const summary = {
			...
			averageCaloriesPerProduct:
				totalItems > 0 ? totalCalories / totalItems : 0,
			averageProteinsPerProduct:
				totalItems > 0 ? totalProteins / totalItems : 0,
			averageCarbsPerProduct: totalItems > 0 ? totalCarbs / totalItems : 0,
			averageFatPerProduct: totalItems > 0 ? totalFat / totalItems : 0,

			// Indicadores de qualidade
			qualityIndicators: {
				highSodiumPercentage:
					totalItems > 0 ? (highSodiumProducts / totalItems) * 100 : 0,
				highSugarPercentage:
					totalItems > 0 ? (highSugarProducts / totalItems) * 100 : 0,
				highSaturatedFatPercentage:
					totalItems > 0 ? (highSaturatedFatProducts / totalItems) * 100 : 0,
				highTransFatPercentage:
					totalItems > 0 ? (highTransFatProducts / totalItems) * 100 : 0,
				highFiberPercentage:
					totalItems > 0 ? (highFiberProducts / totalItems) * 100 : 0,
				highProteinPercentage:
					totalItems > 0 ? (highProteinProducts / totalItems) * 100 : 0,
			},
		};
