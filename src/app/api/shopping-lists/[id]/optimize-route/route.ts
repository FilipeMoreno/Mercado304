...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      optimizedRoute,
      totalEstimatedSavings: optimizedRoute.reduce((sum, market) => 
        sum + market.estimatedSavings, 0
      ),
      summary: {
        totalMarkets: optimizedRoute.length,
        totalItems: shoppingList.items.length,
        itemsDistributed: optimizedRoute.reduce((sum, market) => 
          sum + market.items.length, 0
        )
      }
    })
