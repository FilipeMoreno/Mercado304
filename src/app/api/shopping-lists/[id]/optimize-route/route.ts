...
export async function GET(
	...
	try {
		...
		return NextResponse.json({
			...
			optimizedRoute,
			totalEstimatedSavings: optimizedRoute.reduce(
			...
			summary: {
				...
				itemsDistributed: optimizedRoute.reduce(
					(sum, market) => sum + market.items.length,
					0,
				),
			},
		});
	...
	items.forEach((item) => {
		...
		Object.keys(marketPrices).forEach((marketId) => {
			marketPrices[marketId].price =
				marketPrices[marketId].price / marketPrices[marketId].recordCount;
		});
		...
		if (bestMarketId && marketItems[bestMarketId]) {
			...
			const savings =
				averagePrice > 0 ? (averagePrice - bestPrice) * item.quantity : 0;
