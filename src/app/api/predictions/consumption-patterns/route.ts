...
export async function GET() {
	try {
		...
		const productConsumption = purchases.reduce((acc: any, item) => {
			...
			acc[productId].purchases.push({
				...
				daysSinceEpoch: Math.floor(
					item.purchase.purchaseDate.getTime() / (1000 * 60 * 60 * 24),
				),
			});
		...
		const patterns = Object.values(productConsumption)
			.map((product: any) => {
				const purchases = product.purchases.sort(
					(a: any, b: any) => a.daysSinceEpoch - b.daysSinceEpoch,
				);
				...
				for (let i = 1; i < purchases.length; i++) {
					intervals.push(
						purchases[i].daysSinceEpoch - purchases[i - 1].daysSinceEpoch,
					);
				}
				...
				const daysUntilNextPurchase =
					nextPurchaseExpected - Math.floor(Date.now() / (1000 * 60 * 60 * 24));

				// Calcular urgência (0-100)
				const urgencyFactor = Math.max(
					0,
					(avgInterval - daysSinceLastPurchase) / avgInterval,
				);
				...
				const isNearRepurchase =
					daysUntilNextPurchase <= 7 && daysUntilNextPurchase >= -3;
				...
				return {
					...
					consumption: {
						...
						nextPurchaseExpected: new Date(
							nextPurchaseExpected * 24 * 60 * 60 * 1000,
						),
						...
						isNearRepurchase,
						shouldReplenish:
							isRegularPurchase && hasRegularPattern && isNearRepurchase,
						confidence:
							isRegularPurchase && hasRegularPattern
								? Math.min(95, 60 + purchases.length * 5)
								: 30,
					},
				};
			})
		...
		const sortedPatterns = patterns.sort(
			(a: any, b: any) => b.consumption.urgency - a.consumption.urgency,
		);

		// Produtos que precisam de reposição
		const replenishmentAlerts = sortedPatterns.filter(
			(p: any) => p.consumption.shouldReplenish,
		);

		// Estatísticas gerais
		const stats = {
			totalProductsAnalyzed: patterns.length,
			productsWithRegularPattern: patterns.filter(
				(p: any) => p.consumption.hasRegularPattern,
			).length,
			...
			avgConsumptionInterval:
				patterns.length > 0
					? Math.round(
							patterns.reduce(
								(sum: number, p: any) => sum + p.consumption.avgIntervalDays,
								0,
							) / patterns.length,
						)
					: 0,
		...
		return NextResponse.json(
			{ error: "Erro ao analisar padrões de consumo" },
			{ status: 500 },
		);
	}
}
