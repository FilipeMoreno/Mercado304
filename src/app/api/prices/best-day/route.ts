...
export async function POST(request: Request) {
	try {
		...
		allData.forEach((item) => {
			...
			dayAnalysis[dayOfWeek as keyof typeof dayAnalysis].prices.push(
				item.price,
			);
		...
		const dayStats = Object.keys(dayAnalysis)
			.map((dayKey) => {
				...
				const avgPrice =
					day.prices.length > 0
						? day.prices.reduce((sum, price) => sum + price, 0) /
							day.prices.length
						: 0;
		...
		const monthlyAnalysis = allData.reduce(
			(acc, item) => {
		...
		if (savingsPercentage >= 10) {
			insights.push(
				`Você pode economizar até ${savingsPercentage.toFixed(1)}% comprando na ${bestDay.name}`,
			);
		}
		...
		if (monthlyStats.length > 1) {
			...
			insights.push(
				`Historicamente, ${bestMonth.name} tem os melhores preços para este produto`,
			);
		}
		...
		if (recordsPercentage > 50) {
			insights.push(
				"Análise baseada principalmente em registros de preços coletados",
			);
