...
export async function POST(request: Request) {
	try {
		...
		const dayAnalysis = purchaseItems.reduce((acc: any, item) => {
			const dayOfWeek = getDay(parseISO(item.purchase.purchaseDate.toISOString())) // 0 = Domingo, 1 = Segunda, etc.
		...
		return NextResponse.json({ error: "Erro ao analisar dia da semana" }, { status: 500 })
	}
}
