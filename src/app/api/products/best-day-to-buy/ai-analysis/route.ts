...
export async function POST(request: Request) {
	try {
		...
		if (purchaseItems.length < 3) {
			// Poucos dados para uma análise útil
			return NextResponse.json({
				analysis:
					"Ainda não tenho dados suficientes para analisar o melhor dia de compra para este item.",
			});
		}
		...
		const analysisData = Object.entries(dayAnalysis).map(
			([day, data]: any) => ({
		...
		const bestDay = analysisData.reduce((prev, curr) =>
			prev.averagePrice < curr.averagePrice ? prev : curr,
		);
