...
export async function GET(
	...
	try {
		...
		const lastPurchaseDate =
			purchaseItems.length > 0 ? purchaseItems[0].purchase.purchaseDate : null;

		// Calcular variação de preço (últimos 30 dias vs anteriores)
		const thirtyDaysAgo = new Date();
		thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

		const recentItems = purchaseItems.filter(
			(item) => new Date(item.purchase.purchaseDate) >= thirtyDaysAgo,
		);
		const olderItems = purchaseItems.filter(
			(item) => new Date(item.purchase.purchaseDate) < thirtyDaysAgo,
		);

		const recentAveragePrice =
			recentItems.length > 0
				? recentItems.reduce((sum, item) => sum + item.unitPrice, 0) /
					recentItems.length
				: 0;
		const olderAveragePrice =
			olderItems.length > 0
				? olderItems.reduce((sum, item) => sum + item.unitPrice, 0) /
					olderItems.length
				: 0;
		...
		const recentPriceHistory = purchaseItems.filter(
			(item) => new Date(item.purchase.purchaseDate) >= threeMonthsAgo,
		);
