...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!session?.user || session.user.id !== userId) {
      return NextResponse.redirect(
        new URL("/conta/seguranca?error=reauth_failed&message=Sessão inválida", request.url)
      )
    }
    ...
    return NextResponse.redirect(
      new URL("/conta/seguranca?error=reauth_failed&message=Erro inesperado", request.url)
    )
  }
}
