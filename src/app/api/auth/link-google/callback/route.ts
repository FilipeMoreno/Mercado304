...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action !== "link") {
      return NextResponse.redirect(
        new URL("/conta/seguranca?error=link_failed&message=Ação inválida", request.url)
      )
    }
    ...
    if (Date.now() - timestamp > 10 * 60 * 1000) {
      return NextResponse.redirect(
        new URL("/conta/seguranca?error=link_expired&message=Tempo expirado", request.url)
      )
    }
    ...
    if (!session?.user || session.user.id !== userId) {
      return NextResponse.redirect(
        new URL("/conta/seguranca?error=link_failed&message=Sessão inválida", request.url)
      )
    }
    ...
    return NextResponse.redirect(
      new URL("/conta/seguranca?error=link_failed&message=Erro inesperado", request.url)
    )
  }
}
