...
export async function POST(_request: NextRequest) {
	try {
		...
		if (credentialAccount?.password) {
			return NextResponse.json({ error: "Use o endpoint padrão para contas com senha" }, { status: 400 })
		}
		...
		return NextResponse.json({ error: error.message || "Erro ao habilitar 2FA" }, { status: 500 })
	}
}
