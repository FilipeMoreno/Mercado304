	...
	try {
		...
		if (!preferences) {
			return NextResponse.json({
				...
				showPaymentTotalAmount: false,
				showPaymentAverageTicket: false,
				showPaymentMostUsed: false,
				showPaymentDistribution: false,
				showPaymentDetails: false,
				showPaymentInsights: false,
				customTitle: null,
				customSubtitle: null,
				widgetLayouts: {},
				gridColumns: 12,
		...
		return NextResponse.json({
			...
			showPaymentTransactions: preferences.showPaymentTransactions ?? false,
			showPaymentTotalAmount: preferences.showPaymentTotalAmount ?? false,
			showPaymentAverageTicket: preferences.showPaymentAverageTicket ?? false,
			showPaymentMostUsed: preferences.showPaymentMostUsed ?? false,
			showPaymentDistribution: preferences.showPaymentDistribution ?? false,
			showPaymentDetails: preferences.showPaymentDetails ?? false,
			showPaymentInsights: preferences.showPaymentInsights ?? false,
	...
	try {
		...
		const preferences = await prisma.dashboardPreference.upsert({
			...
			update: {
				...
				showPaymentTransactions: data.showPaymentTransactions ?? false,
				showPaymentTotalAmount: data.showPaymentTotalAmount ?? false,
				showPaymentAverageTicket: data.showPaymentAverageTicket ?? false,
				showPaymentMostUsed: data.showPaymentMostUsed ?? false,
				showPaymentDistribution: data.showPaymentDistribution ?? false,
				showPaymentDetails: data.showPaymentDetails ?? false,
				showPaymentInsights: data.showPaymentInsights ?? false,
