	...
	try {
		...
		const markets = Object.values(marketPrices).map((marketData: any) => {
			...
			const recentData = sortedData.filter(
				(item: any) => new Date(item.date) >= thirtyDaysAgo,
			);

			const olderData = sortedData.filter(
				(item: any) => new Date(item.date) < thirtyDaysAgo,
			);
			...
			if (recentData.length > 0 && olderData.length > 0) {
				const recentAvg =
					recentData.reduce((sum: number, item: any) => sum + item.price, 0) /
					recentData.length;
				const olderAvg =
					olderData.reduce((sum: number, item: any) => sum + item.price, 0) /
					olderData.length;
			...
			const purchaseCount = sources.filter(
				(s: string) => s === "purchase",
			).length;
