...
export async function POST(request: Request) {
	try {
		...
		const markets = Object.values(marketPrices).map((marketData: any) => {
			...
			const recentData = sortedData.filter((item: any) => new Date(item.date) >= thirtyDaysAgo)

			const olderData = sortedData.filter((item: any) => new Date(item.date) < thirtyDaysAgo)
		...
		return NextResponse.json({ error: "Erro ao comparar preços" }, { status: 500 })
	}
}
