...
export async function POST(request: Request) {
	try {
		...
		if (!listId) {
			return NextResponse.json({ error: "List ID é obrigatório" }, { status: 400 })
		}
		...
		const productIds = shoppingList.items.filter((item) => item.productId).map((item) => item.productId!)
		...
		const marketComparisons = markets.map((market) => {
			const marketItems = shoppingList.items
				.map((listItem) => {
					if (!listItem.productId) return null

					// Buscar o preço mais recente deste produto neste mercado
					const recentPurchase = purchaseItems.find(
						(purchase) => purchase.productId === listItem.productId && purchase.purchase.market.id === market.id,
					)
		...
		return NextResponse.json({ error: "Erro ao comparar preços da lista" }, { status: 500 })
	}
}
