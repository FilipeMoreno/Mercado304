...
export async function POST(request: Request) {
	try {
		...
		const marketsWithData = marketComparisons.filter(
			(m) => m.availableItems > 0,
		);
		const cheapestTotal =
			marketsWithData.length > 0
				? Math.min(...marketsWithData.map((m) => m.totalPrice))
				: 0;
		...
		const bestMarket =
			marketsWithData.length > 0
				? marketsWithData.find((m) => m.totalPrice === cheapestTotal)
				: null;
		...
		return NextResponse.json({
			...
			analysis: {
				...
				avgCompletionRate:
					marketsWithSavings.reduce((sum, m) => sum + m.completionRate, 0) /
					marketsWithSavings.length,
			},
		});
	} catch (error) {
