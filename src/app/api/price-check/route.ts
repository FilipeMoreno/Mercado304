...
export async function POST(request: Request) {
	try {
		...
		const marketAverages = Object.values(marketPrices).map((market: any) => ({
			...
			avgPrice: market.prices.reduce((sum: number, price: number) => sum + price, 0) / market.prices.length,
		...
		const overallAverage = marketAverages.reduce((sum, market) => sum + market.avgPrice, 0) / marketAverages.length
		const overallDifference = ((currentPrice - overallAverage) / currentPrice) * 100
		...
		return NextResponse.json({ error: "Erro ao verificar preços históricos" }, { status: 500 })
	}
}
