	...
	try {
		...
		if (!productId || !listId || !itemId) {
			return NextResponse.json(
				{ error: "productId, listId e itemId são obrigatórios." },
				{ status: 400 },
			);
		}
		...
		const lastPurchaseDate = new Date(
			purchases[purchases.length - 1].purchase.purchaseDate,
		);
		...
		if (
			daysSinceLastPurchase > avgIntervalDays * 0.8 &&
			avgQuantityPerPurchase > 1
		) {
