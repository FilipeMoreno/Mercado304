...
function isBlockedQuery(message: string): {
	blocked: boolean;
	reason?: string;
} {
	...
	for (const pattern of dangerousPatterns) {
		if (pattern.test(lowerMessage)) {
			return {
				blocked: true,
				reason:
					"Pergunta sobre informações técnicas do sistema não permitida por segurança.",
			};
		}
	}
	...
	if (suspiciousCount >= 2) {
		return {
			blocked: true,
			reason:
				"Pergunta com múltiplos termos técnicos não permitida por segurança.",
		};
	}
	...
	if (securityCheck.blocked) {
		...
		const stream = new ReadableStream({
			start(controller) {
				...
				controller.enqueue(
					encoder.encode(
						`data: ${JSON.stringify({ content: securityMessage, error: true, final: true })}\n\n`,
					),
				);
				controller.close();
			},
	...
	const stream = new ReadableStream({
		async start(controller) {
			try {
				...
				controller.enqueue(
					encoder.encode(`data: ${JSON.stringify({ final: true })}\n\n`),
				);
			} catch (error) {
...
const toolFunctions = {
	...
	getConsumptionPatterns: async () => {
		const response = await fetch(
			`${process.env.NEXTAUTH_URL}/api/predictions/consumption-patterns`,
		);
		const data = await response.json();
	...
	checkBestPrice: async ({ productName }: { productName: string }) => {
		const response = await fetch(
			`${process.env.NEXTAUTH_URL}/api/best-price-check`,
			{
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ productName }),
	...
	promptChurrascoCalculator: async () => {
		return {
			...
			message:
				"Claro! Vamos calcular tudo para o seu churrasco. Por favor, preencha os detalhes abaixo.",
			options: {},
		};
	},
	calculateChurrasco: async ({
		adults,
		children,
		drinkers,
		preferences,
		...
		try {
			const response = await fetch(
				`${process.env.NEXTAUTH_URL}/api/ai/churrascometro`,
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
			...
			Object.entries(data.shoppingList).forEach(
	...
	try {
		...
		if (message.startsWith("SELEÇÃO_FEITA:")) {
			const selectionData = JSON.parse(
				message.replace("SELEÇÃO_FEITA:", "").trim(),
			);
		...
		if (message.startsWith("CALCULATE_CHURRASCO:")) {
			const churrascoData = JSON.parse(
				message.replace("CALCULATE_CHURRASCO:", "").trim(),
			);
...
Você pode fazer TUDO que o aplicativo permite através das interfaces!`,
		...
		if (error instanceof Error) {
			...
			if (errorText.includes("overloaded") || errorText.includes("503")) {
				errorMessage =
					"O assistente está sobrecarregado no momento. Tentei algumas vezes mas não consegui processar sua solicitação. Tente novamente em alguns segundos.";
			} else if (
				errorText.includes("rate limit") ||
				errorText.includes("429")
			) {
				errorMessage =
					"Muitas solicitações foram feitas recentemente. Aguarde alguns segundos e tente novamente.";
			} else if (errorText.includes("timeout")) {
				errorMessage =
					"A solicitação demorou muito para ser processada. Tente reformular sua pergunta ou tente novamente.";
			} else if (
				errorText.includes("api key") ||
				errorText.includes("unauthorized")
			) {
				errorMessage =
					"Problema de autenticação com o serviço de IA. Entre em contato com o administrador.";
			} else if (
