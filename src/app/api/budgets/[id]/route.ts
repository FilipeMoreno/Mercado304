...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	try {
		...
		if (budget.type === "CATEGORY") {
			// OTIMIZADO: Buscar categoria e itens em uma transação
			const [category, items] = await prisma.$transaction([
			...
			// OTIMIZADO: Buscar mercado e compras em uma transação
			const [market, marketPurchases] = await prisma.$transaction([
			...
			// OTIMIZADO: Buscar produto e itens em uma transação
			const [product, items] = await prisma.$transaction([
