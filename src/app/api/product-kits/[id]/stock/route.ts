...
export async function GET(_request: NextRequest, props: { params: Promise<{ id: string }> }) {
	...
	try {
		const stockInfo = await productKitService.checkKitStockAvailability(params.id)
		...
		return NextResponse.json(
			{
				success: false,
				error: error instanceof Error ? error.message : "Erro ao verificar estoque do kit",
			},
			{ status: 500 },
		)
	}
...
export async function POST(request: NextRequest, props: { params: Promise<{ id: string }> }) {
	...
	try {
		...
		const result = await productKitService.consumeKitFromStock(params.id, body.quantity, body.reason)
		...
		return NextResponse.json(
			{
				success: false,
				error: error instanceof Error ? error.message : "Erro ao consumir kit do estoque",
			},
			{ status: 500 },
		)
	}
