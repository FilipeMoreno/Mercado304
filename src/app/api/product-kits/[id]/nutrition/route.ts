...
export async function GET(_request: NextRequest, props: { params: Promise<{ id: string }> }) {
	...
	try {
		const nutritionalInfo = await productKitService.calculateKitNutritionalInfo(params.id)
		...
		return NextResponse.json(
			{
				success: false,
				error: error instanceof Error ? error.message : "Erro ao calcular informações nutricionais",
			},
			{ status: 500 },
		)
	}
