...
import { ArrowLeft, Grid3X3, ListChecks, Plus, Save, Store, Trash2 } from "lucide-react"
...
export default function NewBudgetPage() {
	...
	const handleTypeChange = (newType: QuoteType) => {
		...
		// Limpar itens se mudar de BY_ITEMS para outro tipo
		if (newType !== QuoteType.BY_ITEMS) {
			setItems([])
		}
		// Limpar valores fixos se mudar para BY_ITEMS
		if (newType === QuoteType.BY_ITEMS) {
			setTotalAmount(0)
			setDiscountAmount(0)
		}
	...
	const updateItem = (tempId: string, field: keyof FormQuoteItem, value: any) => {
		setItems(
			items.map((item) => {
				...
				// Recalcular valores se quantidade, preço ou desconto mudarem
	...
	const selectProduct = (tempId: string, productId: string) => {
		...
		setItems(
			items.map((item) => {
				...
				// Pegar preço médio ou último preço do produto se disponível
	...
	const calculateTotals = () => {
		// Para BY_CATEGORY e BY_MARKET, usar os valores fixos
		...
		// Para BY_ITEMS, calcular a partir dos itens
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		// Para BY_CATEGORY e BY_MARKET, validar valor total
		if (quoteType === QuoteType.BY_CATEGORY || quoteType === QuoteType.BY_MARKET) {
			if (totalAmount <= 0) {
			...
			// Criar um item genérico para representar o valor total
			const categoryName = categories.find((c) => c.id === categoryId)?.name || "Categoria"
			const marketName = markets.find((m) => m.id === marketId)?.name || "Mercado"
			...
			try {
				const result = await createMutation.mutateAsync(budgetData as any)
				router.push(`/cotacoes/${result.id}`)
			} catch (_error) {
				// Error_errorratado pelo mutation
			}
			return
		}

		// Para BY_ITEMS, usar os itens normalmente
		const budgetData = {
			name,
			description: description || undefined,
			type: quoteType,
		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">Crie um nova cotação para comparar preços</p>
				</div>
			...
			<form onSubmit={handleSubmit} className="space-y-6">
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>Dados gerais da cotação</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
								...
								>
									<CardContent className="flex items-center gap-3 p-4">
										...
										<div className="flex-1">
											...
											<div className="text-xs text-muted-foreground">Cotação de itens individuais</div>
										</div>
									</CardContent>
								</Card>
						...
						<div className="space-y-2">
							...
							<p className="text-xs text-muted-foreground">
								{status === "DRAFT"
									? "Cotação em rascunho pode ser editado posteriormente"
									: "Cotação finalizada não pode ser alterada"}
							</p>
						</div>
					</CardContent>
				</Card>
				...
				{(isByCategory || isByMarket) && (
					<Card>
						<CardHeader>
							<CardTitle>Valor do Cotação</CardTitle>
							<CardDescription>
								{isByCategory
