...
import { Search, TrendingDown, TrendingUp, Target, ShoppingCart, MapPin, Loader2, ChevronDown, ChevronUp, Check, X, Command } from "lucide-react"
...
export function ComparisonClient({ initialLists, initialMarkets, initialProducts, searchParams }: ComparisonClientProps) {
  ...
  const compareDetailedList = async () => {
    ...
    try {
      const response = await fetch('/api/price-comparison/detailed-list', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ listId: detailedListId, marketIds: selectedMarketIds })
      });
      if (response.ok) {
        const data = await response.json();
                          ...
                          >
                            <div>
                              ...
                              <div className="text-sm text-gray-600 flex items-center gap-2 mt-1">
                                ...
                                <span className="ml-2">Última compra: {new Date(market.lastPurchase).toLocaleDateString('pt-BR')}</span>
                              </div>
                            </div>
                    ...
                    {productComparison.markets.length > 1 && (
                      <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                        ...
                        <p className="text-blue-600 text-sm">
                          Comprando no <strong>{getBestPrice(productComparison.markets)?.marketName}</strong>, você economiza
            ...
            <Card>
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="flex justify-end">
                        <Button onClick={compareDetailedList} disabled={loadingDetailed || selectedMarketIds.length < 2 || !detailedListId}>
                            {loadingDetailed ? (
                                <Loader2 className="h-4 w-4 animate-spin" />
                            ) : (
