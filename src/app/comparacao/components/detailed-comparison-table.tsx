							...
							{detailedComparison.markets.map((market) => (
								<th key={market.id} className="p-3 text-sm font-medium">
									<div className="flex flex-col">
										...
										{market.location && <span className="text-xs text-gray-500 font-normal">{market.location}</span>}
									</div>
								</th>
							))}
						</tr>
							...
							<tr key={productItem.product?.id} className="border-b last:border-b-0">
								...
								{productItem.comparison.map((comp) => (
									<td key={comp.marketId} className="p-3">
										{comp.price !== null ? (
											<div className="flex flex-col">
												<span className={cn("font-semibold", comp.isCheapest ? "text-green-600" : "text-foreground")}>
													R$ {comp.price.toFixed(2)}
												</span>
												{comp.saving > 0 && (
													<span className="text-xs text-red-500">(Economia: R$ {comp.saving.toFixed(2)})</span>
												)}
											</div>
										) : (
				...
				{detailedComparison.products.map((productItem) => (
					<div key={productItem.product?.id} className="border rounded-lg p-4 bg-white">
						<div className="mb-3">
							...
							{productItem.product?.brand && <p className="text-xs text-gray-500">{productItem.product.brand.name}</p>}
						</div>
								...
								return (
									<div key={comp.marketId} className="flex justify-between items-center p-2 bg-gray-50 rounded-sm">
										<div className="flex-1 min-w-0">
											...
											{market?.location && <div className="text-xs text-gray-500 truncate">{market.location}</div>}
										</div>
