...
import { Users, Beer, Drumstick, ShoppingCart, Loader2, Sparkles, History, Utensils, GlassWater, Flame, Calculator, Clock } from "lucide-react"
...
export default function ChurrascoPage() {
  ...
  const handleCreateList = async () => {
    ...
    const itemsToCreate = Object.values(result.shoppingList)
      .flat()
      .map(item => ({ productName: item.item, quantity: 1, isChecked: false }));
      
    await create('/api/shopping-lists', {
                    ...
                    <div key={category} className="space-y-3">
                      ...
                      <div className="space-y-2">
                        {items.map((item, index) => (
                          <div key={`${item.item}-${index}`} className="flex items-center justify-between p-2 bg-muted/30 rounded-lg">
